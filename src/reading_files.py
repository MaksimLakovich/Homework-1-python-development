...
def get_transactions_from_csv(file_path: Path, delimiter=";") -> List[dict]:
    ...
    list_transactions_from_csv = df.apply(lambda row:
                                          {
                                              "id": row["id"],
                                              "state": row["state"],
                                              "date": row["date"],
...
def get_transactions_from_excel(file_path: Path) -> List[dict]:
    ...
    list_transactions_from_excel = df.apply(lambda row:
                                            {
                                                "id": row["id"],
                                                "state": row["state"],
                                                "date": row["date"],
